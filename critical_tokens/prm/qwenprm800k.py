...
class QwenPRM800K:
    def __init__(self, device="cuda", batch_size=32):
        self.device = device
        self.tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen2.5-Math-7B-PRM800K", trust_remote_code=True)
        self.model = AutoModel.from_pretrained("Qwen/Qwen2.5-Math-7B-PRM800K", device_map=device, trust_remote_code=True)
